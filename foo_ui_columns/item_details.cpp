    ...
    uih::ScrollAxis axis, int new_position, bool supress_smooth_scroll, bool quick_animation)
{
    if (config::use_smooth_scrolling && !supress_smooth_scroll) {
        m_smooth_scroll_helper->absolute_scroll(axis, new_position, quick_animation);
        return;
    }
...
void ItemDetails::delta_scroll(uih::ScrollAxis axis, int delta, bool supress_smooth_scroll, bool quick_animation)
{
    if (config::use_smooth_scrolling && !supress_smooth_scroll) {
        m_smooth_scroll_helper->delta_scroll(axis, delta, quick_animation);
        return;
    }
...
void ItemDetails::internal_scroll(uih::ScrollAxis axis, int position, bool is_delta)
{
    ...
    current_si.fMask = SIF_POS | SIF_TRACKPOS | SIF_PAGE | SIF_RANGE;
    ...
    if (new_pos != current_si.nPos) {
        SCROLLINFO new_si{};
        new_si.cbSize = sizeof(current_si);
