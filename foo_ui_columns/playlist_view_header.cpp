...
void playlist_view::move_header(bool redraw, bool update)
{
    ...
    if (rc_header.left != 0 - horizontal_offset || rc_header.top != 0
        ...
        if (rc_header.bottom - rc_header.top != header_height) {
            ...
            ScrollWindowEx(
                wnd_playlist, 0, (header_height - rc_header.bottom), &playlist, &playlist, nullptr, &redraw, 0);
...
{
    if (wnd_header) {
        DestroyWindow(wnd_header);
        wnd_header = nullptr;
    }
    if (cfg_header) {
        wnd_header = CreateWindowEx(0, WC_HEADER, _T("Playlist display column titles"),
            WS_CHILD | (visible ? WS_VISIBLE : 0) | HDS_HOTTRACK | HDS_DRAGDROP | HDS_HORZ
                | (/*nohscroll ? 0 : */ HDS_FULLDRAG) | (cfg_header_hottrack ? HDS_BUTTONS : 0),
