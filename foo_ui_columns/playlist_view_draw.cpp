...
bool playlist_view::draw_items_wrapper(int start_item, int count)
{
    ...
    if ((visible_last - visible_first + 1) * item_height < rect.bottom - rect.top)
        visible_last++;

    if (visible_last >= total)
        visible_last = total - 1;

    if (start_item + count - 1 < visible_first || start_item > visible_last)
        return false;
...
bool playlist_view::draw_items(HDC dc, int start_item, int count)
{
    ...
    for (int n = start_item; n < end_item; n++) {
        ...
        for (int c = 0; c < t; c++) {
            if (p_mask[c]) {
                ...
                if (b_themed) {
                    ...
                    // GetThemeColor(m_theme, LVP_LISTITEM, sel ? (GetFocus() == wnd_playlist ? LIS_SELECTED :
                    // LIS_SELECTEDNOTFOCUS) : LIS_NORMAL, TMT_WINDOWTEXT, &colourfore);
                    colourfore = GetThemeSysColor(m_theme, sel ? COLOR_BTNTEXT : COLOR_WINDOWTEXT);
                    // GetThemeColor(m_theme, LVP_LISTITEM, sel ? (GetFocus() == wnd_playlist ? LIS_SELECTED :
                    // LIS_SELECTEDNOTFOCUS) : LIS_NORMAL, TMT_TEXTCOLOR, &colourfore);
                    if (!theme_state) {
                ...
                // render text
                // if (b_themed)
                //    DrawThemeText(m_theme, hdc_mem, LVP_LISTITEM, sel ? (GetFocus() == wnd_playlist ? LIS_SELECTED :
                //    LIS_SELECTEDNOTFOCUS) : LIS_NORMAL, L"test", 4, 0, 0, &draw);
                // else
                uih::text_out_colours_tab(hdc_mem, temp, temp.length(), 2, 1, &draw, sel, colourfore, TRUE, true,
                    (cfg_ellipsis != 0), (uih::alignment)columns[c]->align);
        ...
        if ((n + scroll_item_offset) == focus) {
            if (m_always_show_focus
                || (!(SendMessage(get_wnd(), WM_QUERYUISTATE, NULL, NULL) & UISF_HIDEFOCUS) && b_focused)) {
                RECT rc_focus = item;
