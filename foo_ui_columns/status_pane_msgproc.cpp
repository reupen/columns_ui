...
LRESULT StatusPane::on_message(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_PAINT: {
        ...
        const auto simple_text_opts
            = uih::direct_write::TextOutOptions{.enable_style_codes = false, .enable_tab_columns = false};

        uih::direct_write::text_out_columns_and_styles(*m_text_format, wnd, dc.get(), mmh::to_string_view(items_text),
            1_spx, 3_spx, rc_line_1, default_text_colour, simple_text_opts);

        if (m_item_count) {
            uih::direct_write::text_out_columns_and_styles(*m_text_format, wnd, dc.get(),
