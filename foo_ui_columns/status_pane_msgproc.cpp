...
LRESULT status_pane::on_message(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_WINDOWPOSCHANGED: {
        ...
        if (!(lpwp->flags & SWP_NOSIZE)) {
            ...
            SetWindowPos(m_volume_control.get_wnd(), nullptr, lpwp->cx - vol_cx - 4, 2 + (lpwp->cy - vol_cy) / 2,
                vol_cx, vol_cy, SWP_NOZORDER);
    ...
    case WM_PAINT: {
        ...
        uih::text_out_colours_tab(
            dc, items_text, -1, 4, 2, &rc_top, false, default_text_colour, false, false, false, uih::ALIGN_LEFT);
        if (m_item_count) {
            pfc::string_formatter formatter;
            uih::text_out_colours_tab(dc, formatter << "Length: " << m_length_text, -1, 4, 2, &rc_bottom, false,
                default_text_colour, false, false, false, uih::ALIGN_LEFT);
        }

        if (m_menu_active) {
            {
                RECT rc_item = rc_top;
                uih::text_out_colours_tab(dc, m_menu_text, -1, 4 + placeholder_len, 0, &rc_item, false,
                    default_text_colour, false, false, false, uih::ALIGN_LEFT, nullptr, true, true, nullptr);
            }
        } else {
            placeholder = "Playing:  ";
            ...
            t_size now_playing_x_end = 4 + placeholder_len + placeholder2_len;
            {
                ...
                pfc::string_formatter formatter;
                uih::text_out_colours_tab(dc, formatter << m_track_label << "  ", -1, 4 + placeholder_len, 0, &rc_item,
            ...
            if (playing1.get_length()) {
                ...
                if (lines)
                    uih::text_out_colours_tab(dc, playingstrings[0], pfc_infinite, now_playing_x_end, 0, &rc_top, false,
                        default_text_colour, true, true, false, uih::ALIGN_LEFT);
                if (lines > 1)
                    uih::text_out_colours_tab(dc, playingstrings[1], pfc_infinite, now_playing_x_end, 0, &rc_bottom,
