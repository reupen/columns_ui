...
namespace filter_panel {
        ...
        for (size_t i{0}; i < count; i++) {
            ...
            if (exact_match) {
                ...
                if (selected && handles_count)
                    tracks_for_next_window.add_items_fromptr(m_nodes[index_item].m_handles.get_ptr() + current_count, handles_count);
            }
            else {
                node_t node;
    ...
    size_t filter_panel_t::make_data_entries(const metadb_handle_list_t<pfc::alloc_fast_aggressive>& tracks, pfc::list_t<data_entry_t, pfc::alloc_fast_aggressive> & p_out, bool b_show_empty)
    {
        ...
        if (m_field_data.m_use_script) {
            ...
            concurrency::parallel_for(size_t{0}, track_count, [&node_count, &to, tracks_ptr, b_show_empty, pp_out](size_t i) {
            ...
            concurrency::parallel_for(size_t{0}, track_count, [&out_counter, tracks_ptr, p_infos, b_show_empty, pp_out](size_t i) {
