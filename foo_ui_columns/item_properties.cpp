...
const GUID & selection_properties_t::get_extension_guid() const {return g_guid_selection_properties;}
void selection_properties_t::get_name(pfc::string_base & p_out)const {p_out = "Item properties";}
void selection_properties_t::get_category(pfc::string_base & p_out)const {p_out = "Panels";}
unsigned selection_properties_t::get_type () const {return uie::type_panel;}
...
void selection_properties_t::on_selection_changed(const pfc::list_base_const_t<metadb_handle_ptr> & p_selection)
{
    ...
    if (check_process_on_selection_changed())
    {
        ...
        if (m_tracking_mode == track_nowplaying || (m_tracking_mode == track_automatic && static_api_ptr_t<play_control>()->is_playing()))
            return;
...
{
    m_handles.remove_all();
    if (m_tracking_mode == track_selection || (m_tracking_mode == track_automatic && !static_api_ptr_t<play_control>()->is_playing()))
    {
        m_handles = m_selection_handles;
    }
    else if (m_tracking_mode == track_nowplaying || (m_tracking_mode == track_automatic && static_api_ptr_t<play_control>()->is_playing()))
    {
...
namespace
{
    ...
    font_header_client_selection_properties::factory<font_header_client_selection_properties> g_font_header_client_selection_properties;
