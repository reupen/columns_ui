		...
		if (cmd>0)
		{
			...
			else
			{
				...
				case ID_CUT:
					...
					break;
				case ID_COPY:
					if (b_index_valid) playlist_manager_utils::copy(pfc::list_single_ref_t<t_size>(index));
					break;
				case ID_PASTE:
					if (b_index_valid) playlist_manager_utils::paste(get_wnd(), index+1);
					break;
				case ID_SWITCH:
					if (b_index_valid) m_playlist_api->set_active_playlist(index);							
					break;
				case ID_REMOVE:
					if (b_index_valid) m_playlist_api->remove_playlist_switch(index);								
					break;
				case ID_RENAME:
					if (b_index_valid)
					{
