...
void playlist_view::create_inline_edit_v2(const pfc::list_base_const_t<t_uint32>& indices, unsigned column)
{
    ...
    if (!start_visible || !end_visible) {
        ...
        if (indices_count > si.nPage) {
            target = median;
            scroll(scroll_vertically, scroll_position_delta,
                target - scroll_item_offset - ((si.nPage > 1 ? si.nPage - 1 : 0) / 2));
        } else {
            ...
            scroll(scroll_vertically, scroll_position_delta,
                target - scroll_item_offset
                    - (target > gsl::narrow<unsigned>(scroll_item_offset) ? (si.nPage > 1 ? si.nPage - 1 : 0) : 0));
        }
    }
    ...
    if (x - horizontal_offset + cx > rc_playlist.right)
        scroll(scroll_horizontally, scroll_position_delta,
            x - horizontal_offset + (cx > rc_playlist.right ? 0 : cx - rc_playlist.right));
    ...
    if (!m_wnd_edit) {
        ...
        m_wnd_edit = CreateWindowEx(0, WC_EDIT, pfc::stringcvt::string_os_from_utf8(text).get_ptr(),
            WS_CHILD | WS_CLIPSIBLINGS | WS_VISIBLE | ES_LEFT | ES_AUTOHSCROLL | ES_MULTILINE | ES_AUTOVSCROLL
                | WS_BORDER | WS_CLIPCHILDREN,
            x, y, cx, cy, wnd_playlist, HMENU(667), core_api::get_my_instance(), nullptr);
        ...
        SetWindowPos(m_wnd_edit, nullptr, x, y, cx, cy, SWP_NOZORDER);
        // if (ptr)
        ...
        case VK_TAB: {
            ...
            if (count && indices_count) {
                ...
                if (back) {
                    ...
                    do {
                        while (column > 0
                            && !(found = !g_get_columns()[g_get_cache().active_column_active_to_actual(--column)]
                                              ->edit_field.is_empty())) {
                        }
                    } while (!found && indices_count == 1 && playlist_index > 0 && (column = count)
                        && (playlist_index-- >= 0));
