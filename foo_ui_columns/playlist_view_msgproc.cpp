...
LRESULT playlist_view::on_message(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_PAINT: {
        ...
        if (rc_update.bottom >= rc_update.top) {
            ...
            if (((end_item - start_item) + 1) * item_height < rc_update.bottom - rc_update.top)
                end_item++;
            {
    ...
    case WM_CHAR:
        ...
        break;
    case WM_KEYUP:
        if (process_keydown(msg, lp, wp, true))
            return 0;
        break;
    case WM_SYSKEYUP:
        if (process_keydown(msg, lp, wp, true))
    ...
    case WM_LBUTTONDOWN: {
        ...
        //        if( idx >= 0 && idx <playlist_api->activeplaylist_get_item_count()  && GET_X_LPARAM(lp) <
        //        g_playlist_entries.get_total_width_actual())
        ...
        if (idx >= 0) {
            ...
            if (wp & MK_CONTROL && wp & MK_SHIFT) {
                ...
                drag_type = 2;
                dragitem = idx, dragstartitem = idx;
                ...
                drag_type = 2;
                dragitem = idx, dragstartitem = idx; // item irrelevant actually;
    ...
    case WM_LBUTTONUP: {
        ...
        g_drag_lmb = false;
        int idx = hittest_item(GET_X_LPARAM(lp), GET_Y_LPARAM(lp),
            true); //((GET_Y_LPARAM(lp) -get_header_height()) / get_item_height()) + scroll_item_offset;
        static_api_ptr_t<playlist_manager> playlist_api;
        if (!dragged) {
            if (wp & MK_CONTROL) {
                //            int idx_down = hittest_item(drag_start_lmb.x, drag_start_lmb.y);
                if (idx >= 0)
                    set_sel_single(idx, true, true, false);
            } else {
                //                int item_height = get_item_height();
                ...
                {
                    if (!m_no_next_edit && cfg_inline_edit && playlist_api->activeplaylist_is_item_selected(idx)
                        && m_prev_sel /*&& !dragged*/) {
    ...
    case WM_MOUSEMOVE: {
        ...
        if (drag_type && (wp & MK_LBUTTON) && !(GetKeyState(VK_SHIFT) & KF_UP) && !(GetKeyState(VK_CONTROL) & KF_UP)) {
            ...
            if ((idx + scroll_item_offset) != dragitem
                || GET_Y_LPARAM(lp)
                    < get_header_height()) //(idx + scroll_item_offset) < playlist_api->activeplaylist_get_item_count()
            {
                if (idx >= items_count - 1) {
                    ...
                    if (old_offset != scroll_item_offset)
                        need_redrawing = true;
                    ...
                    if (t != dragitem) {
                        drawing_enabled = false;
                        if (drag_type == 1)
                            playlist_api->activeplaylist_move_selection(
                                (rc.bottom - rc.top) / item_height + scroll_item_offset - focus - 1);
                        else if (drag_type == 2) {
    ...
    break;
    case WM_MOUSEWHEEL: { // GET_WHEEL_DELTA_WPARAM
        exit_inline_edit();
    ...
    case WM_CONTEXTMENU: {
        ...
        if ((HWND)wp == wnd_header) {
            ...
            if (!(hittest.flags & HHT_NOWHERE)) {
                ...
                uAppendMenu(menu, (MF_SEPARATOR), 0, "");
                uAppendMenu(menu, (MF_STRING | (cfg_nohscroll ? MF_CHECKED : MF_UNCHECKED)), IDM_AUTOSIZE,
                    "&Auto-sizing columns");
                ...
                for (int s = 0; s < e; s++) {
                    ...
                    if (add) {
                        uAppendMenu(menu, MF_STRING | (columns[s]->show ? MF_CHECKED : MF_UNCHECKED),
                            IDM_CUSTOM_BASE + s, columns[s]->name);
                    }
                }
        ...
        if ((HWND)wp == wnd) {
            ...
            if (playlist_api->activeplaylist_get_selection_count(1) > 0 && true) {
                if (pt.x == -1 && pt.y == -1) {
                    ...
                    if (focus == (std::numeric_limits<size_t>::max)() || focus < gsl::narrow<size_t>(scroll_item_offset)
                        || focus > last) {
                        px.x = 0;
                        px.y = 0;
                    } else {
                ...
                if (p_manager_selection.is_valid()) {
                    ...
                    p_manager_selection->generate_menu_win32(menu, ID_SELECTION, ID_CUSTOM_BASE - ID_SELECTION,
            ...
            //    contextmenu_manager::win32_run_menu_context_playlist(wnd, 0,
            //    config_object::g_get_data_bool_simple(standard_config_objects::bool_show_keyboard_shortcuts_in_menus,
            //    true) ? contextmenu_manager::FLAG_SHOW_SHORTCUTS : 0);
        }
    }
        return 0;
    ...
    case WM_NOTIFY:
        ...
        case ID_PLAYLIST_TOOLTIP:
            ...
            case TTN_SHOW:
                ...
                SetWindowPos(g_tooltip, nullptr, rc.left, rc.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
                return TRUE;
            }
            break;
        case 5001:
            ...
            case HDN_ENDDRAG: {
                if (((LPNMHEADERA)lp)->iButton == 0) {
                    if (((LPNMHEADERA)lp)->pitem && (((LPNMHEADERA)lp)->pitem->mask & HDI_ORDER)) {
                        ...
                        if (to >= 0 && from != to) {
                            int act_from = g_cache.active_column_active_to_actual(from),
                                act_to = g_cache.active_column_active_to_actual(to);
            ...
            case HDN_ITEMCLICK: {
                ...
                g_set_sort(((LPNMHEADER)lp)->iItem,
                    des /*, playlist_api->activeplaylist_get_selection_count(1) && cfg_sortsel != 0*/);

            } break;
            case HDN_ITEMCHANGED: {
                if (!cfg_nohscroll) {
                    if (((LPNMHEADER)lp)->pitem->mask & HDI_WIDTH)
                        columns[g_cache.active_column_active_to_actual(((LPNMHEADER)lp)->iItem)]->width
                            = ((LPNMHEADER)lp)->pitem->cxy;
