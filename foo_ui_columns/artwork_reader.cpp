...
const std::unordered_map<GUID, album_art_data_ptr>& ArtworkReader::get_previous_artwork_data() const
{
    return m_previous_artwork_data;
}
...
void ArtworkReaderManager::request(
    const metadb_handle_ptr& handle, OnArtworkLoadedCallback on_artwork_read, const bool is_from_playback)
{
    const auto read_stub_images = m_stub_images.empty();
    abort_current_task();

    m_current_reader = std::make_shared<ArtworkReader>(
        m_artwork_type_ids, m_previous_artwork_data, is_from_playback, on_artwork_read);
