...
class FontsDataSet : public fcl::dataset {
    ...
    enum Identifier {
        ...
        identifier_use_alternative_emoji_processing,
        ...
        abort_callback& p_abort) const override
    {
        ...
        out.write_item(identifier_use_alternative_emoji_processing, fonts::use_alternative_emoji_processing);
        ...
        abort_callback& p_abort) override
    {
        ...
        while (reader.get_remaining()) {
            ...
            case identifier_use_colour_glyphs:
                ...
                break;
            case identifier_use_alternative_emoji_processing:
                reader.read_item(fonts::use_alternative_emoji_processing);
                break;
            case identifier_colour_emoji_font_family: {
                pfc::string8 value;
                reader.read_item(value, element_size);
                value = std::move(fonts::colour_emoji_font_family);
                break;
            }
            case identifier_monochrome_emoji_font_family: {
                pfc::string8 value;
                reader.read_item(value, element_size);
                value = std::move(fonts::monochrome_emoji_font_family);
                break;
            }
            default:
