...
void playlist_view_cache::on_items_added(unsigned p_playlist, unsigned start,
...
void playlist_view_cache::on_items_reordered(unsigned p_playlist, const unsigned* order, unsigned count)
{
    ...
    for (n = 0; n < count; n++) {
        ...
        if (n > start)
            on_items_change(p_playlist, pfc::bit_array_range(start, n));
    }
}
...
void playlist_view_cache::on_items_modified_fromplayback(
...
void playlist_view_cache::get_display_name(unsigned playlist, unsigned idx, int col, pfc::string_base& out)
{
    ...
    if (idx >= 0 && idx < p_cache->get_count()) {
        if (!p_cache->get_item(idx)) {
            if (!update_item(playlist, idx)) {
                out.set_string("Error");
                return;
            }
        }
        p_cache->get_item(idx)->get_item(col)->get_display(out);
        return;
    }
...
void playlist_view_cache::make_extra(
    unsigned playlist, unsigned idx, global_variable_list& p_out, const SYSTEMTIME* st, bool b_legacy) const
{
    ...
    playlist_api->playlist_item_format_title(
        playlist, idx, &tf_hook, extra_formatted, g_to_global, nullptr, play_control::display_level_all);
...
bool playlist_view_cache::update_item(unsigned playlist, unsigned idx)
{
    ...
    if (idx >= 0 && idx < p_cache->get_count()) {
        ...
        colourinfo colours_global(p_helper.get_colour(cui::colours::colour_text),
            ...
            if (p_mask[s]) {
                ...
                if (to_display.is_valid()) {
                    ...
                    titleformat_hook_splitter_pt3 tf_hook(
                        global ? &tf_hook_set_global : nullptr, date ? &tf_hook_date : nullptr, &tf_hook_playlist_name);

                    playlist_api->playlist_item_format_title(
                        playlist, idx, &tf_hook, display, to_display, nullptr, play_control::display_level_all);
                }

                colourinfo col_item(p_helper.get_colour(cui::colours::colour_text),
                    p_helper.get_colour(cui::colours::colour_selection_text),
                    p_helper.get_colour(cui::colours::colour_background),
                    p_helper.get_colour(cui::colours::colour_selection_background),
                    p_helper.get_colour(cui::colours::colour_inactive_selection_text),
                    ...
                    if (!colour_global_av) {
                        ...
                        if (to_global_style.is_valid()) {
                            ...
                            titleformat_hook_splitter_pt3 tf_hook(&tf_hook_style,
                                global ? &tf_hook_set_global : nullptr, date ? &tf_hook_date : nullptr,
                                &tf_hook_playlist_name);

                            playlist_api->playlist_item_format_title(playlist, idx, &tf_hook, colour, to_global_style,
                ...
                if (b_custom) {
                    if (to_colour.is_valid()) {
                        ...
                        titleformat_hook_splitter_pt3 tf_hook(&tf_hook_style, global ? &tf_hook_set_global : nullptr,
                            date ? &tf_hook_date : nullptr, &tf_hook_playlist_name);
...
int playlist_view::get_column_width(unsigned column1) const
{
    ...
    int rv = 0;
    if (column1 >= 0 && column1 < widths.get_size())
        rv = widths[column1];
...
bool playlist_view_cache::get_playlist_sort(unsigned playlist, unsigned& idx, bool* p_descending)
{
    playlist_cache* p_cache = get_item(playlist);
    if (p_descending)
        *p_descending = p_cache->m_sorted_descending;
...
class asc_sort_callback : public pfc::list_base_t<T>::sort_callback {
    int compare(const T& p_item1, const T& p_item2) override
    {
        ...
        if (temp == 0)
            temp = (p_item1->index > p_item2->index ? 1 : -1);
        return temp;
    }
};
...
class desc_sort_callback : public pfc::list_base_t<T>::sort_callback {
    int compare(const T& elem1, const T& elem2) override
    {
        ...
        if (temp == 0)
            temp = (elem1->index > elem2->index ? 1
                                                : -1); // we could swap -1 and 1 for desc, but then, apart from not
                                                       // making any sense, same sort repeated would yield diff results
        return temp;
    }
};
...
void playlist_view::g_set_sort(unsigned column, bool descending, bool selection_only)
{
    ...
    if (active_playlist != -1
        && (!playlist_api->playlist_lock_is_present(active_playlist)
               || !(playlist_api->playlist_lock_get_filter_mask(active_playlist) & playlist_lock::filter_reorder))) {
        ...
        for (n = 0; n < count; n++) {
            if (!selection_only || mask[n] == true) {
                //        if (custom_sort)
                {
                    ...
                    titleformat_hook_splitter_pt3 tf_hook(
                        extra ? &tf_hook_set_global : nullptr, date ? &tf_hook_date : nullptr, &tf_hook_playlist_name);

                    playlist_api->activeplaylist_item_format_title(
                        n, &tf_hook, temp, to_sort, nullptr, play_control::display_level_none);
                }
                /*        else
                        {
...
bool titleformat_hook_set_global<set, get>::process_field(
...
template <bool set, bool get>
bool titleformat_hook_set_global<set, get>::process_function(titleformat_text_out* p_out, const char* p_name,
