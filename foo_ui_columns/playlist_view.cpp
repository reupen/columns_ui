...
{
    //    if (idx_rel + scroll_item_offset < playlist_api->activeplaylist_get_item_count() && idx_rel +
    //    scroll_item_offset >=0 && col >= 0
...
LRESULT playlist_view::CreateToolTip(const char* text)
{
    ...
    g_tooltip = CreateWindowEx(b_comctl_6 ? WS_EX_TRANSPARENT : 0, TOOLTIPS_CLASS, nullptr,
        WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP | TTS_NOPREFIX, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
        CW_USEDEFAULT, wnd_playlist, nullptr, core_api::get_my_instance(), nullptr);
...
public:
    void on_items_added(unsigned p_index, unsigned start, const pfc::list_base_const_t<metadb_handle_ptr>& p_data,
        const pfc::bit_array& p_selection)
    ...
    void FB2KAPI on_items_removed(
        unsigned p_index, const pfc::bit_array& p_mask, unsigned p_old_count, unsigned p_new_count) override
    {
    ...
    void on_items_selection_change(
        unsigned p_index, const pfc::bit_array& affected, const pfc::bit_array& state) override
    {
    ...
    void on_items_modified(unsigned p_index, const pfc::bit_array& p_mask, bool b_playback,
        playback_control::t_display_level p_level = playback_control::display_level_none)
    {
...
playlist_view::class_data& playlist_view::get_class_data() const
{
    ...
    __implement_get_class_data_ex(_T("{F20BED8F-225B-46c3-9FC7-454CEDB6CDAD}"), _T(""), false, 0,
        WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_TABSTOP, flags, CS_DBLCLKS | CS_HREDRAW);
}
