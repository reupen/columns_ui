...
public:
	...
	column_base_t(const char * pname, const char * pspec, bool b_use_custom_colour, const char * p_colour_spec, bool b_use_custom_sort, const char * p_sort_spec, unsigned p_width, alignment p_align, playlist_filter_type p_filter_type, const char * p_filter_string, unsigned p_parts, bool b_show, const char * p_edit_field)
		: name(pname), spec(pspec), use_custom_colour(b_use_custom_colour), colour_spec(p_colour_spec), use_custom_sort(b_use_custom_sort), sort_spec(p_sort_spec), width(p_width), align(p_align), filter_type(p_filter_type), filter(p_filter_string), parts(p_parts), show(b_show), edit_field(p_edit_field)
	{};
...
public:
	...
	column_t() /*: idx(0)*/ {};
	column_t(const char * pname, const char * pspec, bool b_use_custom_colour, const char * p_colour_spec, bool b_use_custom_sort, const char * p_sort_spec, unsigned p_width, alignment p_align, playlist_filter_type p_filter_type, const char * p_filter_string, unsigned p_parts, bool b_show, const char * p_edit_field)
		: column_base_t(pname, pspec, b_use_custom_colour ,p_colour_spec, b_use_custom_sort, p_sort_spec, p_width, p_align, p_filter_type, p_filter_string, p_parts, b_show, p_edit_field)
	{};
private:
...
public:
	...
	void set_entries_copy (column_list_cref_t entries)
	{
		//remove_all();
		t_size i, count = entries.get_count();
		set_count(count);
		for (i=0; i<count; i++)
			(*this)[i] = new column_t(*entries[i].get_ptr());
	}
