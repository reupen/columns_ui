...
INT_PTR TextRenderingTab::on_message(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_INITDIALOG: {
        ...
        if (m_is_font_fallback_available) {
            Button_SetCheck(m_use_custom_emoji_processing_checkbox,
                fonts::use_alternative_emoji_processing ? BST_CHECKED : BST_UNCHECKED);
        ...
        if (!m_is_font_fallback_available) {
            const auto checkbox_text
                = uGetWindowText(m_use_custom_emoji_processing_checkbox) + " (requires Window 8.1 or newer)";
            uSetWindowText(m_use_custom_emoji_processing_checkbox, checkbox_text.c_str());
        }
    ...
    case WM_COMMAND:
        ...
        case IDC_USE_CUSTOM_EMOJI_PROCESSING: {
            fonts::use_alternative_emoji_processing
                = Button_GetCheck(m_use_custom_emoji_processing_checkbox) == BST_CHECKED;
            enable_or_disable_emoji_controls();
            g_font_manager_data.dispatch_all_fonts_changed();
            break;
        }
...
void TextRenderingTab::enable_or_disable_emoji_controls() const
{
    ...
    EnableWindow(
        m_colour_emoji_family_combobox, m_is_font_fallback_available && fonts::use_alternative_emoji_processing);
    EnableWindow(
        m_monochrome_emoji_family_combobox, m_is_font_fallback_available && fonts::use_alternative_emoji_processing);
}
