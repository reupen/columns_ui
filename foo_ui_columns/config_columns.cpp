...
void speedtest(ColumnListCRef columns, bool b_global)
{
...

    {
        for (size_t i = 0; i < column_count; i++)
            if (!columns[i]->def.use_custom_colour) {
            ...
            for (unsigned n = 0; n < column_count; n++) {
                {
                    ...
                    for (i = 0; i < 10; i++) {
                        ...
                        pfc::hires_timer timer;
                        timer.start();
                        titleformat_api->compile_safe(times_columns[n].to_display, columns[n]->def.spec);
                        time_temp += timer.query();
                    }
                ...
                }
                {
                    ...
                    if (columns[n]->def.use_custom_colour) {
                        double time_temp = 0;
                        unsigned i;
                        for (i = 0; i < 10; i++) {
                            ...
                            pfc::hires_timer timer;
                            timer.start();
                            titleformat_api->compile_safe(times_columns[n].to_colour, columns[n]->def.colour_spec);
                            time_temp += timer.query();
                        }
