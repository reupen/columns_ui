...
namespace filter_panel {
    ...
    public:
        ...
        static bool g_filter_search_bar_has_stream(filter_search_bar const* p_seach_bar,
                                                   const filter_stream_t* p_stream);
        ...
        static void g_initialise_filter_stream(const TStream& p_stream)
        {
            for (t_size i = 0, count = g_active_instances.get_count(); i < count; i++) {
                if (!cfg_orderedbysplitters 
                        || g_filter_search_bar_has_stream(g_active_instances[i], p_stream.get_ptr())) {
        ...
        {
            return cui::toolbars::guid_filter_search_bar;
        }
        ...
        t_uint32 get_flags() const override
        {
            return flag_default_flags_plus_transparent_background;
        }
