...
static const GUID font_client_cph_guid = 
...
struct fontsizeinfo {bool up; INT size; INT new_size; INT caps; bool changed;};
...
int fontsizecommonproc(int pointsize, DWORD FontType, fontsizeinfo * fn)
{
    if(FontType != TRUETYPE_FONTTYPE)
    {
		if (fn->up)
		{
			if (pointsize > fn->size    &&    (  !(fn->new_size > fn->size) ||   pointsize < fn->new_size)) {fn->new_size = pointsize;}
		}
		else
		{
			if (pointsize < fn->size    &&    pointsize > fn->new_size) {fn->new_size = pointsize;}
		}
		return 1;
    }
	else 
