namespace ThreadUtils {
	static void WaitAbortable(HANDLE ev, abort_callback & abort, DWORD timeout = pfc_infinite) {
		const HANDLE handles[2] = {ev, abort.get_abort_event()};
		SetLastError(0);
		const DWORD status = WaitForMultipleObjects(2, handles, FALSE, pfc_infinite);
		switch(status) {
			case WAIT_OBJECT_0:
				break;
