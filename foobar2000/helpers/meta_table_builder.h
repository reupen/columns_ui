...
public:
	...
	void operator() (const char * p_name,const t_values & p_values) {
		...
		for(t_values::const_iterator iter = p_values.first(); iter.is_valid(); ++iter) {
			if (index == pfc_infinite) index = m_info.__meta_add_unsafe(p_name,*iter);
...
public:
	...
	void add(const char * p_name,const char * p_value,t_size p_value_len = pfc_infinite) {
	...
	void set(const char * p_name,const char * p_value,t_size p_value_len = pfc_infinite) {
		if (file_info::g_is_valid_field_name(p_name)) {
