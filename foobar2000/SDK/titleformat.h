...
public:
	virtual void write(const GUID & p_inputtype,const char * p_data,t_size p_data_length = pfc_infinite) = 0;
...
public:
	...
	bool process_field(titleformat_text_out * p_out,const char * p_name,t_size p_name_length,bool & p_found_flag) {
		if (
			stricmp_utf8_ex(p_name,p_name_length,"list_index",pfc_infinite) == 0
			) {
			p_out->write_int_padded(titleformat_inputtypes::unknown,m_index+1, m_total);
			p_found_flag = true; return true;
		} else if (
			stricmp_utf8_ex(p_name,p_name_length,"list_total",pfc_infinite) == 0
			) {
