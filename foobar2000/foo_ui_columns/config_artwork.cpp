...
public:
	...
	static bool get_separated_index (t_size combined_index, t_size & index, t_size & subindex)
	{
		index = 0;
		subindex = 0;
		while (index < tabsize(g_artwork_sources) && g_artwork_sources[index].m_scripts->get_count() <= combined_index)
		{
	...
	static void get_group_from_combined_index (t_size combined_index, t_size & index, t_size & subindex, t_size & combined_index_start, t_size & count)
	{
	...
	static BOOL CALLBACK g_on_message(HWND wnd,UINT msg,WPARAM wp,LPARAM lp)
	{
		...
		if (msg == WM_INITDIALOG)
		{
			...
			p_data = reinterpret_cast<tab_artwork*>(GetWindowLongPtr(wnd, DWL_USER));
		return p_data ? p_data->on_message(wnd, msg, wp, lp) : FALSE;
	}

	void on_scripts_change()
	{
		m_source_list.on_scripts_change();
	}
	...
	BOOL CALLBACK on_message(HWND wnd,UINT msg,WPARAM wp,LPARAM lp)
	{
		...
		case WM_COMMAND:
			...
			case IDC_EDGESTYLE|(CBN_SELCHANGE<<16):
				...
				artwork_panel::g_on_edge_style_change();
				break;
			case IDC_ADD:
				{
					RECT rc;
