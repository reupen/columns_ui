...
void seek_bar_extension::update_seekbars(bool positions_only)
{
	...
	{
		for (n=0; n<count; n++)
			if (windows[n]->get_wnd()) windows[n]->update_seek_pos();
	}
	else
	{
		for (n=0; n<count; n++)
			if (windows[n]->get_wnd()) windows[n]->update_seek();
	}
...
unsigned seek_bar_extension::get_type  () const{return ui_extension::type_toolbar;};
