...
class status_pane : public ui_helpers::container_window, private playlist_callback_single, play_callback
{
	...
	/** PLAYLIST CALLBACKS */
	void on_items_added(t_size p_base, const pfc::list_base_const_t<metadb_handle_ptr> & p_data,const bit_array & p_selection) {on_playlist_change();}
	...
	void on_items_removed(const bit_array & p_mask,t_size p_old_count,t_size p_new_count) {on_playlist_change();}
	void on_items_selection_change(const bit_array & p_affected,const bit_array & p_state) {on_playlist_change();}
	...
	void on_playlist_switch() {on_playlist_change();}
	...
	/** PLAY CALLBACKS */
	void on_playback_starting(play_control::t_track_command p_command,bool p_paused) {m_track_label = "Opening..."; invalidate_all();}
	void on_playback_new_track(metadb_handle_ptr p_track) {m_track_label = "Playing:"; update_playing_text(); invalidate_all();}
	void on_playback_stop(play_control::t_stop_reason p_reason) {if (p_reason != playback_control::stop_reason_shutting_down) {m_track_label = ""; update_playing_text();  invalidate_all();}}
	void on_playback_seek(double p_time) {update_playing_text();  invalidate_all();}
	void on_playback_pause(bool p_state) {m_track_label = p_state ? "Paused:" : "Playing:"; update_playing_text(); invalidate_all();}
	void on_playback_edited(metadb_handle_ptr p_track) {update_playing_text();  invalidate_all();}
	void on_playback_dynamic_info(const file_info & p_info) {update_playing_text();  invalidate_all();}
	void on_playback_dynamic_info_track(const file_info & p_info) {update_playing_text();  invalidate_all();}
	void on_playback_time(double p_time) {update_playing_text();  invalidate_all();}
